...
export const suggestionService = {
  ...
  getSuggestionById: async (id) => {
    try {
      ...
      console.log('获取建议详情原始响应:', response);
      
      // 直接返回响应数据，不再进行额外的嵌套
      return validateSuggestionData(response);
    } catch (error) {
  ...
  updateSuggestion: async (id, updateData) => {
    try {
      ...
      // 判断是否是FormData类型（包含文件上传）
      const isFormData = updateData instanceof FormData;
      
      // 如果是FormData类型，则不设置Content-Type，让浏览器自动设置
      ...
      const response = await api.put(`/suggestions/${id}`, updateData, config);
      return response;
    } catch (error) {
      console.error('更新建议失败:', error);
      // 增强错误处理，确保将后端返回的错误信息传递出去
      if (error.response && error.response.data) {
        console.error('后端返回错误信息:', error.response.data);
        throw {
          ...error,
          userMessage: error.response.data.userMessage || error.response.data.message || '未知错误'
        };
      }
      throw error;
    }
  ...
  // 部门经理重置建议评分
  resetSuggestionScore: async (id) => {
    try {
      const response = await api.post(`/suggestions/${id}/reset-score`);
      return response; // 返回更新后的 suggestion 对象
    } catch (error) {
      console.error(`建议 ${id} 重置评分失败:`, error);
      throw error; // 将错误抛出给调用者处理
    }
  }
};
...
function validateSuggestionData(suggestion) {
  ...
  // 处理评分信息
  // 如果评分不存在或评分已被完全重置，则scoring保持为undefined或null
  let scoring = data.scoring;
  
  // 处理建议状态
  let reviewStatus = data.reviewStatus || data.status;
  let implementationStatus = data.implementationStatus || 
  ...
  return {
    ...
    scoring,
    // 确保submitter字段存在
    submitter: data.submitter || { name: '未知', _id: null },
    // 确保comments字段是数组
